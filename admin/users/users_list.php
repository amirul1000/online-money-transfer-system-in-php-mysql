				      ...
				      <div class="table-responsive">	
						 ...
						 ?>
							<tr bgcolor="<?=$row?>" onmouseover=" this.style.background='#ECF5B6'; " onmouseout=" this.style.background='<?=$row?>'; ">
                              ...
                              <td><?=$arr[$i]['company']?></td>
                              <td>address:<?=$arr[$i]['address']?><br>
								  city:<?=$arr[$i]['city']?><br>
                                  state:<?=$arr[$i]['state']?><br>
                                  zip:<?=$arr[$i]['zip']?><br>                              
		                          country:
								  <?php
									    unset($info2);        
                               ...
                               <td>
                                    <form action="" method="post">
                                       <?php
											$enum_is_merchant = getEnumFieldValues('users','is_merchant');
										?>
										<select  name="is_merchant" id="is_merchant"   class="form-control-static">
										<?php
										   foreach($enum_is_merchant as $key=>$value)
										   { 
										?>
										  <option value="<?=$value?>" <?php if($value==$arr[$i]['is_merchant']){ echo "selected"; }?>><?=$value?></option>
										 <?php
										  }
										?> 
										</select>
                                        ...
                                        <input type="hidden" name="cmd" value="change_is_merchant">
                               ...
                               <td>
                                    <?php
                                        unset($info2);        
                                        $info2['table']    = designation;	
                                        $info2['fields']   = array("name");	   	   
                                        $info2['where']    =  "1 AND id='".$arr[$i]['designation_id']."' LIMIT 0,1";
                                        $res2  =  $db->select($info2);
                                        echo $res2[0]['name'];	
                                    ?>
                               </td>
