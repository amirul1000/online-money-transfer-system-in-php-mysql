<?php
	   ...
	   switch($cmd)
	   {
	      ...
	      case "order" :  
		   ...
		   break;	
		  case "order_balance":  
		        $balace = get_balance($db,$_SESSION['users_id']);
				if($balace>=$_REQUEST['payment_amuont'])
				{
					order_add($db);
									
					 unset($arr);
					$arr['users_id']= $_SESSION['users_id'];
					$arr['subject']= 'Purchase';
					$arr['description']= 'Purchase';
					$arr['currency_id']= get_currency_id($db,'USD');
					$arr['debit'] = $_REQUEST['payment_amuont'];
					$arr['credit'] = 0.0;
					$arr['refference']= 'Purchase';
				...
				{
				  $message = " <font color=\"red\">Your balance is less than Payment amount</font>";	
				}
	        include("cart_list.php");
		   break;	  
		 case "update":
...
function order_add($db)
{
	...
	//add order
	   ...
	   unset($data);
	$info['table']    = "orders";
	$data['products_id']   = $_SESSION['cart'][0]['products_id'];
